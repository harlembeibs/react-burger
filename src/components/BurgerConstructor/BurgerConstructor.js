    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      {
        thread.map((item, index) => {
          return <ConstructorElement
            isLocked={index === 0 || index === thread.length - 1 ? true : undefined}
            type={index === 0 ? "top" : index === thread.length - 1 ? "bottom" : undefined} 
            text={item.name}
            price={item.price}
  ...
  render() {
    return (
      <section className={burgerConstructorStyles.container}>
        <Scrollbars style={{ width: 552, height: 630 }} renderThumbVertical={props => <div {...props} className="thumb-vertical" style={{ backgroundColor: '#8585AD', maxHeight: 664 }}/>}>
