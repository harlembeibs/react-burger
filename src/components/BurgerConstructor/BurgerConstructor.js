...
const TotalPrice = ({ thread }) => {
  return (
    thread.reduce((total, value) => {
  ...
  render() {
    return (
      <section className={burgerConstructorStyles.container}>
        {
        this.props.thread.map((item, index) => {
          if (index === 0) {
            return <ConstructorElement
            isLocked={true}
            type="top" 
        ...
        <Scrollbars className={burgerConstructorStyles.scrollBar} renderThumbVertical={props => <div {...props} className={burgerConstructorStyles.thumbVertical} />}>
          <article className={burgerConstructorStyles.itemsContainer}>
            <BurgerConstructorItems thread={this.props.thread} />
          </article>
        </Scrollbars>
        {
        this.props.thread.map((item, index) => {
          if (index === this.props.thread.length - 1) {
            return <ConstructorElement
            isLocked={true}
            type="bottom" 
            text={item.name}
        ...
        <div className={burgerConstructorStyles.totalPriceContainer}>
          <div className={burgerConstructorStyles.totalPrice}>
            <span className={burgerConstructorStyles.totalPriceText}>{<TotalPrice thread={this.props.thread} />}</span>
